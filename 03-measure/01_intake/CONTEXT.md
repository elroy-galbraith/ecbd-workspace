...
5. Count the matrix with `python _tools/conform.py --check <source>`, which reports the counts and which statistics they support. Record them here and in `RUN.md` frontmatter `items:` and `models:`. **Say what will not be computable**, rather than discovering it at stage 3.
