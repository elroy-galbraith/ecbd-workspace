...
4. Run the traceability test against the capabilities from Q3, **from the sources**: does each one have items targeting it, assembly covering it, and accumulation measuring it? Name every capability that drops out, and every number reported with no capability behind it. This is a documentation check, not a measurement.
5. Strength labels on Q17 and Q20.
6. **If the benchmark has OpenEval coverage, stop and say so.** Record that item-level data exists, name the split, and state which SUPPORT questions a measure run could answer. Do not compute here — `03-measure/` owns that job, has its own inputs and its own gates, and an audit that quietly does psychometrics inline is two pipelines pretending to be one. Leave the affected labels at `none` and let the measure run revise them.
