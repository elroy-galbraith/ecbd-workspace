...
- Reference (every run): ../../_shared/results-contract.md — what your runner must emit for `03-measure/` to ingest
...
4. Write `build/README.md`: how to run it, what it emits, and the traceability table. State the results format explicitly, following `_shared/results-contract.md` — a build whose output cannot be ingested by `03-measure/` can never have its validity gaps closed by measurement.
